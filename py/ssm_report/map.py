...
def make_serum_coverage_index(output_dir, lab, virus_type, assay, pdf_dir):
    ...
    def make_image(f, filename, safari):
        filename = "{}-{}/{}".format(virus_type, assay, Path(filename).name)
        if safari:
    ...
    for safari in [False, True]:
        ...
        with index_filename.open("w") as f:
            f.write(sHead % {"title": "Serum Coverage {} {} {}".format(lab.upper(), virus_type.upper(), assay.upper()), "width": 400, "height": 415})
                ...
                if mod[0] != "?":
                    ...
                    if pdf.exists():
                        ...
                        if mod_data.get("N") == "comment" and mod_data.get("type") == "data":
                            f.write("<h3>SR {serum_no} {serum_name}<br>AG {antigen_no} {antigen_name}<br>Titer: {titer}</h3>\n".format(**mod_data))
                            f.write("<table class=\"serum-coverage\"><tr><td>All theoretical: {theoretical}</td><td> 12 months theoretical: {theoretical}</td><td>All empirical: {empirical}</td><td> 12 months empirical: {empirical}</td></tr><tr>\n".format(**mod_data))
                            make_image(f=f, filename=str(pdf), safari=safari)
                            make_image(f=f, filename=str(pdf).replace(".all-", ".12m-"), safari=safari)
                            make_image(f=f, filename=str(pdf).replace(".theoretical.", ".empirical."), safari=safari)
                            make_image(f=f, filename=str(pdf).replace(".theoretical.all-", ".empirical.12m-"), safari=safari)
                            f.write("</tr></table>\n")
                        else:
                            f.write("<h3>{}</h3>\n".format(pdf.stem))
            f.write("</body></html>\n")
