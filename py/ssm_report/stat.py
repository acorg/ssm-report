...
def make_stat(stat_dir, hidb_dir, force=False):
    if force or not stat_dir.joinpath("index.html").exists():
        report_settings = json.load(Path("report.json").open())
        if report_settings["previous"]:
            previous_dir = Path(report_settings["previous"]).expanduser().resolve()
            previous_stat_dir = previous_dir.joinpath("stat")
            if not previous_stat_dir.exists():
                previous_stat_dir = previous_dir.joinpath("maps", "stat")
        else:
            previous_stat_dir = None
        if previous_stat_dir.exists():
            module_logger.info('previous_stat_dir {}'.format(previous_stat_dir))
        else:
            module_logger.warning('****** NO previous_stat_dir {}'.format(previous_stat_dir))
        settings_dates = report_settings["time_series"]["date"]
        end_date = (datetime.datetime.strptime(settings_dates["end"], "%Y-%m-%d").date() + datetime.timedelta(days=31)).strftime("%Y-%m-01")
        _make_stat(output_dir=stat_dir, hidb_dir=hidb_dir, start=settings_dates["start"], end=end_date, previous_stat_dir=previous_stat_dir,
                       make_all_names=False, make_tabs=False, make_csv=False, make_webpage=True)

# ----------------------------------------------------------------------

def _make_stat(output_dir, hidb_dir, start, end, previous_stat_dir, make_all_names=False, make_tabs=True, make_csv=True, make_webpage=True):
...
def _load_previous_stat(previous_stat_dir):
    ...
    if previous_stat_path and previous_stat_path.exists():
        ...
        previous_stat = read_json(previous_stat_path)
    else:
        previous_stat = None
    ...
    with filename.open('w') as output:
        ...
        for virus_type in sVirusTypeOrder:
            output.write('<li><span class="flu-type" style="font-weight: bold;">{virus_type}</span> {links}</li>\n'.format(
                virus_type=virus_type.replace('all', 'All flu types,'), links=' '.join('<a href="#{virus_type}-{lab}">{lab}</a>'.format(virus_type=virus_type, lab=sLabDisplayName[lab.upper()]) for lab in ['all'] + sLabOrder)))
        output.write('</ul>\n')
        output.write('<a href="stat.csv">Yearly statistics in the CSV format</a>\n')
        for virus_type in sVirusTypeOrder:
            for lab in ['all'] + sLabOrder:
                output.write('<hr />\n<h3 id="{virus_type}-{lab}" style="margin-bottom: 5px;"><span class="flu-type">{virus_type}</span> {lab}</h3>\n'.format(virus_type=virus_type.replace('all', 'All flu types,'), lab=sLabDisplayName[lab.upper()]))
